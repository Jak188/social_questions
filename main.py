...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        ...
        if u[5]==1: # Blocked (Rule 3, 19, 47)
            await update.message.reply_text(f"ከadmin በመጣ ትእዛዝ መሰረት ለጊዜው ታግደዋል @penguiner ን ያናግሩ")
            return

        # Rules Check (Rule 29, 30, 35, 62, 63)
        ...
        if chat.type=="private" and cmd.startswith("/") and cmd not in priv_cmds and user.id not in ADMIN_IDS:
            ...
            await admin_notify(context, f"🚫 <b>Auto Block</b>\nተማሪ: {user.first_name}\nID: {user.id}\nምክንያት: ያልተፈቀደ ትእዛዝ")
            return
        ...
        if chat.type!="private" and cmd.startswith("/") and cmd not in group_cmds and user.id not in ADMIN_IDS:
            ...
            await update.message.reply_text(f"⚠️ {user.first_name} ያልተፈቀደ ትእዛዝ በመጠቀምዎ 3.17 ነጥብ ተቀንሶ ለ 17 ደቂቃ ታግደዋል")
            await admin_notify(context, f"⚠️ <b>Mute</b>\nID: <code>{user.id}</code>\nGroup: {chat.title}\nለመፍታት /unmute2")
            return

        # Quiz Start (Rule 10-15, 25, 39, 40)
        if cmd in ["/start2","/history_srm2","/geography_srm2","/mathematics_srm2","/english_srm2"]:
            if GLOBAL_STOP and user.id not in ADMIN_IDS:
                await update.message.reply_text(f"ከ admin በመጣ ትእዛዝ ቦቱ ቆሟል @penguiner ን ያናግሩ")
                return
            ...
            now_t = datetime.now()
            await db.execute("INSERT OR REPLACE INTO active_paths VALUES(?,?,?,?)", (chat.id, chat.title or "Private", user.first_name, now_t.strftime("%H:%M")))
            await db.commit()
...
async def admin_ctrl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Reply Smart ID Extraction (Rule 64)
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        if cmd=="approve" and target_id:
        ...
        elif cmd in ["block","block2"] and target_id:
            await db.execute("UPDATE users SET is_blocked=1 WHERE user_id=?",(target_id,))
            await update.message.reply_text("Blocked 🚫")
        elif cmd in ["unblock","unblock2"] and target_id:
            ...
            await update.message.reply_text(f"ተማሪ ({r[2] if r else 'ተማሪ'}) እገዳዎ በአድሚኑ ትእዛዝ ተነስቶልዎታል በድጋሚ ላለመሳሳት ይሞክሩ")
        elif cmd=="oppt":
            global GLOBAL_STOP
            GLOBAL_STOP=True
            await broadcast(context, "ከ admin በመጣ ትእዛዝ ቦቱ ለጊዜው ተቆጥቧል")
        elif cmd=="opptt":
            GLOBAL_STOP=False
            ...
            async with db.execute("SELECT user_id, name, is_blocked, muted_until FROM users WHERE is_blocked=1 OR muted_until IS NOT NULL") as c:
                res="🚫 <b>Block/Mute List</b>\n"
                for r in await c.fetchall():
                    st = "blocked" if r[2]==1 else "muted"
                    res+=f"{r[1]} | ID: <code>{r[0]}</code> | {st}\n"
...
# ===================== NOTIFICATIONS (Rule 9, 60, 61) =====================
async def status_notif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = update.my_chat_member
    u = update.effective_user
    txt = f"✅ ተማሪ {u.first_name} ቦቱን አብርቷል" if m.new_chat_member.status=="member" else f"❌ ተማሪ {u.first_name} ቦት አጥፍቷል"
    await admin_notify(context, f"{txt}\nID: {u.id}\n@penguiner")
...
def main():
    ...
    app_bot.add_handler(CommandHandler(["approve","anapprove","block","block2","unblock","unblock2","unmute2","log","clear_log","oppt","opptt","info","hmute","pin","clear_rank2","close","keep","keep2"], admin_ctrl))
