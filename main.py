...
app = Flask('')
@app.route('/')
def home(): return "Strict Quiz System Online!"
...
# ===================== DATABASE SETUP (Rule 7, 18, 45, 53) =====================
async def init_db():
...
# ===================== CORE LOGIC: POINT SYSTEM (Rule 28, 38) =====================
async def send_quiz(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Rule 10, 11, 12, 13, 14, 27: Subject logic
...
async def receive_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        ...
        # Rule 28 & 38: Point System (8 for first, 4 for late, -1.5 for wrong)
...
# ===================== THE RULES ENGINE (Rule 1, 2, 3, 4, 5, 29, 30, 35, 62, 63, 65) =====================
async def filter_and_handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        ...
        if cmd.startswith("/"):
            if chat.type == "private" and cmd not in priv_allowed and user.id not in ADMIN_IDS:
                ...
                for a in ADMIN_IDS: await context.bot.send_message(a, f"🚫 <b>Block Alert (Private Violation)</b>\nተማሪ: {user.first_name}\nID: <code>{user.id}</code>\nምክንያት: ያልተፈቀደ ትእዛዝ {cmd}\n{ADMIN_USERNAME}")
                return
            ...
            if chat.type != "private" and cmd not in group_allowed and user.id not in ADMIN_IDS:
                ...
                for a in ADMIN_IDS: await context.bot.send_message(a, f"⚠️ <b>Mute Alert</b>\nተማሪ: {user.first_name}\nID: <code>{user.id}</code>\nቦታ: {chat.title}\nለመፍታት replay /unmute2 በል\n{ADMIN_USERNAME}")
                return

        # Start/Stop Logic (Rule 10-15, 25, 31, 37, 39, 40, 42)
        if cmd in ["/start2","/history_srm2","/geography_srm2","/mathematics_srm2","/english_srm2"]:
            ...
            await db.commit()
            for a in ADMIN_IDS: await context.bot.send_message(a, f"🚀 <b>ውድድር ተጀመረ</b>\nበ: {user.first_name}\nቦታ: {chat.title or 'Private'}\nሰዓት: {now_t.strftime('%H:%M')}\n{ADMIN_USERNAME}")
            ...
            await db.commit()
            for a in ADMIN_IDS: await context.bot.send_message(a, f"🏁 <b>ውድድር ቆመ</b>\nቦታ: {chat.title or 'Private'}\n{ADMIN_USERNAME}")

        elif cmd == "/rank2": # Rule 17, 44
            async with db.execute("SELECT name, points FROM users ORDER BY points DESC LIMIT 20") as c:
                res = "📊 <b>ደረጃ እና ነጥብ</b>\n"
                for i,r in enumerate(await c.fetchall(),1): res+=f"{i}. {r[0]} - {r[1]} pts\n"
                await update.message.reply_text(res, parse_mode="HTML")

# ===================== ADMIN DISPATCH (Rule 16, 21-24, 32-36, 43-59, 64) =====================
async def admin_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Rule 64: Reply logic to extract ID
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        if cmd == "approve" and target_id: # Rule 24, 56
        ...
        elif cmd == "anapprove" and target_id: # Rule 6, 57
            ...
            await update.message.reply_text(f"ተማሪ {r[0] if r else 'ተማሪ'} እገዳዎ በአድሚኑ ትእዛዝ ተነስቶልዎታል በድጋሚ ላለመሳሳት ይሞክሩ")
            await context.bot.send_message(target_id, "✅ እገዳዎ ተነስቷል")

        elif cmd == "oppt": # Rule 21, 58, 61
            global GLOBAL_STOP
            GLOBAL_STOP = True
            await broadcast(context, f"ከ admin በመጣ ትእዛዝ መሰረት ለታወቀ ዝግጅት ቦቱ ተቆጥቧል\nለበለጠ መረጃ {ADMIN_USERNAME} ን ያናግሩ")
            ...
            async with db.execute("SELECT user_id, username, name, is_blocked, muted_until FROM users WHERE is_blocked=1 OR muted_until IS NOT NULL") as c:
                ...
                for r in await c.fetchall():
                    tag = "blocked" if r[3]==1 else "muted"
                    res += f"{r[2]} (@{r[1]}) ID: <code>{r[0]}</code> -> {tag}\n"
                await update.message.reply_text(res or "List Empty", parse_mode="HTML")

        elif cmd == "pin": # Rule 16, 54
            async with db.execute("SELECT user_id, username, name FROM users") as c:
                res = "👥 <b>ተመዝጋቢዎች:</b>\n"
                for r in await c.fetchall(): res += f"{r[2]} (@{r[1]}) - <code>{r[0]}</code>\n"
                await update.message.reply_text(res or "No members", parse_mode="HTML")
            ...
            async with db.execute("SELECT * FROM active_paths") as c:
                ...
                await update.message.reply_text(res or "No active paths")
            ...
            await update.message.reply_text("Closed session")
...
async def on_status_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for a in ADMIN_IDS: await context.bot.send_message(a, f"ተማሪ {u.first_name} ቦቱን {status}\nID: {u.id}\n{ADMIN_USERNAME}")
...
def main():
    ...
    # Message Logic for Users
    app_bot.add_handler(MessageHandler(filters.COMMAND, filter_and_handle))
    
    # Admin Panel
    admin_list = ["approve","anapprove","block","unblock","unmute2","log","clear_log","oppt","opptt","hmute","pin","clear_rank2","close","keep","keep2","info"]
    app_bot.add_handler(CommandHandler(admin_list, admin_commands))
    
    app_bot.add_handler(PollAnswerHandler(receive_answer))
    app_bot.add_handler(ChatMemberHandler(on_status_change, ChatMemberHandler.MY_CHAT_MEMBER))
