...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        ...
        # 1. Registration Logic (Points 1, 5, 41)
        if not u:
            ...
            await db.commit()
            await update.message.reply_text(f"👋 ውድ ተማሪ {user.first_name} የምዝገባ ጥያቄዎ በሂደት ላይ ነው አድሚኑ እስኪቀበልዎ እባክዎ በትእግስት ይጠብቁ")
        ...
        # 2. Security (Points 29, 30, 35, 62, 63)
        ...
        # 3. Competition Commands (Points 10-15, 31, 37, 40)
...
# ===================== ADMIN CONTROLS (Points 16-24, 32-36, 43-59) =====================
async def admin_ctrl(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect('quiz_bot.db') as db:
        if cmd=="approve" and target_id:
            ...
            await context.bot.send_message(target_id, "✅ ምዝገባዎ በአድሚኑ ትእዛዝ ተቀባይነት አግኝቷል")
            await update.message.reply_text(f"Approved ID: {target_id}")
            await admin_notify(context, f"✅ ምዝገባ ጸደቀ: ID {target_id} {ADMIN_USERNAME}")
            ...
            await update.message.reply_text("Rejected and Deleted")

        elif cmd=="block" and target_id:
            await db.execute("UPDATE users SET is_blocked=1 WHERE user_id=?",(target_id,))
            await update.message.reply_text(f"Blocked {target_id}")
            ...
            async with db.execute("SELECT name FROM users WHERE user_id=?",(target_id,)) as c: row = await c.fetchone()
            name = row[0] if row else "ተማሪ"
            ...
            await db.execute("UPDATE users SET points=0"); await update.message.reply_text("ደረጃ እና ነጥብ በአዲስ ተጀምሯል")

        elif cmd=="pin":
            async with db.execute("SELECT user_id, username, name FROM users") as c:
                res="👥 <b>ጠቅላላ ተመዝጋቢዎች</b>\n"
            ...
            async with db.execute("SELECT * FROM active_paths") as c:
                res="🔍 <b>ACTIVE መንገዶች</b>\n"
                for r in await c.fetchall(): res+=f"{r[1]} | {r[2]} | {r[3]}\n"
            await update.message.reply_text(res or "ምንም የለም", parse_mode="HTML")

        elif cmd=="oppt":
            global GLOBAL_STOP
            GLOBAL_STOP=True
            await broadcast(context, f"⛔️ ከ admin በመጣ ትእዛዝ መሠረት ለታወቀ ጊዜ ቦቱ ቆሟል ለበለጠ መረጃ {ADMIN_USERNAME} ን ያናግሩ")
        
        elif cmd=="opptt":
            GLOBAL_STOP=False
            await broadcast(context, f"✅ ቦቱ ተመልሷል ስራ ጀምሯል {ADMIN_USERNAME}")
            ...
            async with db.execute("SELECT user_id, username, name, is_blocked, muted_until FROM users WHERE is_blocked=1 OR muted_until IS NOT NULL") as c:
                res="🔇 <b> የታገዱ (Muted/Blocked)</b>\n"
                for r in await c.fetchall():
                    status = "Blocked" if r[3]==1 else "Muted"
                    res+=f"ID: <code>{r[0]}</code> | {r[1]} ({r[2]}) - <b>{status}</b>\n"
            ...
            async with db.execute("SELECT * FROM users WHERE user_id=?",(target_id,)) as c:
                u = await c.fetchone()
                if u: await update.message.reply_text(f"👤 ስም: {u[2]}\nUser: {u[1]}\nID: <code>{u[0]}</code>\nነጥብ: {u[3]}\nየተመዘገበው: {u[7]}", parse_mode="HTML")
        await db.commit()

# ===================== STATUS NOTIF (Points 9, 60, 61) =====================
async def status_notif(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = update.my_chat_member
    name = update.effective_user.first_name
    if m.new_chat_member.status=="member":
        await admin_notify(context, f"✅ ተማሪ {name} ቦቱን አብርቷል... {ADMIN_USERNAME}")
    else:
        await admin_notify(context, f"❌ ተማሪ {name} ቦቱን አጥፍቷል... {ADMIN_USERNAME}")
